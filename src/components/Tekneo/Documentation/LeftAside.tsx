...
export const LeftAside: React.FC<LeftAsideProps> = ({ title, activeKey, buttons }) => {
  ...
  const verifyTheCorrectViewProduct = useCallback(() => {
    ...
    buttons.forEach((item) => {
      ...
      item.ref.current.dataset.active = activeKey === item.key ? "true" : "false";
    });
  }, [activeKey, buttons]);
        ...
        {buttons.map((btn) => (
          <li key={btn.key} className="w-full">
            <button
              ...
              className={`flex gap-[10px] w-full items-center justify-between p-[8px_12px] btnSection rounded-[8px] text-white ${activeKey === btn.key ? "!text-white !border-black bg-gray-800" : ""}`}
              type="button"
            >
              <span className="text-left">{btn.key}</span>
              <svg xmlns="http://www.w3.org/2000/svg" className="fill-white size-[16px] rotate-90" viewBox="0 -4.5 20 20">
            ...
            {btn.popoverRef ? (
              <Popover ref={btn.popoverRef} gapTop={-(btn.ref?.current?.offsetHeight ?? 100)} gapLeft={(btn.ref?.current?.offsetWidth ?? 180) + 15}>
                {btn.childrenPopover}
              </Popover>
            ) : null}
          </li>
